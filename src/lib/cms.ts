...
async function fetchFromCMS<T>(
    ...
    try {
        const res = await fetchImpl(`${CMS_BASE_URL}${endpoint}`, {
...
export async function getEvents(
    {fetch: fetchImpl}: FetchOptions = {}
): Promise<StoredEvent[]> {
    const data = await fetchFromCMS<StoredEvent[]>('/events', fetchImpl);
    return Array.isArray(data) ? data : fallback.events;
}
...
export async function getEvent(
    ...
    const data = await fetchFromCMS<StoredEvent>(`/events/${slug}`, fetchImpl);
    if (data) return data;
