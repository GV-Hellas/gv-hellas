<script lang="ts">
    ...
    function openMenu(groupKey: string | null) {
...
<nav class="sticky top-0 z-30 border-b border-[#8ea8cf] bg-header text-slate-900 shadow-lg backdrop-blur-md">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-3 lg:px-6">
        ...
        <button class="rounded-lg border border-white/25 p-2 md:hidden" type="button"
        ...
        <div class="hidden items-center gap-3 md:flex">
            <a href="/"
               class="rounded-full px-4 py-2.5 text-base font-extrabold text-slate-900 hover:bg-white/35">{$t('nav.home')}</a>
        ...
        <div class="hidden items-center gap-2 md:flex">
            <div class="flex rounded-full bg-white/10 p-1">
                <button type="button" class="rounded-full px-3 py-1 text-xs font-bold transition"
                        class:bg-white={$locale === 'el'} class:text-primary-dark={$locale === 'el'}
                        onclick={() => switchLanguage('el')}>EL
                </button>
                <button type="button" class="rounded-full px-3 py-1 text-xs font-bold transition"
                        class:bg-white={$locale === 'de'} class:text-primary-dark={$locale === 'de'}
            ...
            <div class="ml-1 flex items-center gap-1">
                {#each socials as social}
                    <a href={social.href} target="_blank" rel="noopener" aria-label={social.name}
                       class="rounded-full p-2 transition hover:bg-white/35">
    ...
    {#if mobileOpen}
        <div class="border-t border-white/15 px-4 pb-4 md:hidden">
            <a href="/"
               class="mt-3 block rounded-lg px-3 py-2 text-sm font-semibold hover:bg-white/30">{$t('nav.home')}</a>
            {#each groups as group}
                <p class="mt-3 px-3 text-xs font-semibold uppercase tracking-wide text-slate-700">{$t(`nav.${group.key}`)}</p>
                {#each group.items as item}
                    <a href={item.href}
                       class={`mt-1 block rounded-lg px-3 py-2 text-sm font-semibold ${isActive(item.href) ? 'bg-white/35' : 'hover:bg-white/30'}`}>
                        {$t(`nav.${item.key}`)}
                    </a>
                {/each}
            {/each}
            <div class="mt-4 flex items-center justify-between px-3">
                <div class="flex gap-2">
                    <button class="rounded bg-white/35 px-3 py-1 text-sm font-semibold" type="button"
                            onclick={() => switchLanguage('el')}>EL
                    </button>
                    <button class="rounded bg-white/35 px-3 py-1 text-sm font-semibold" type="button"
                            onclick={() => switchLanguage('de')}>DE
