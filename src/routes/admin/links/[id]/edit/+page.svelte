...
<h1 class="mb-4 text-2xl font-bold">Edit link</h1>
<form method="POST" enctype="multipart/form-data" action="?/save" class="grid gap-3 rounded border border-slate-200 bg-white p-4 md:grid-cols-2">
  <input name="name_el" value={data.item.name.el} placeholder="Title EL" class="rounded border px-3 py-2" required />
  <input name="name_de" value={data.item.name.de} placeholder="Title DE" class="rounded border px-3 py-2" required />
  <input name="url" type="url" value={data.item.url} placeholder="https://..." class="rounded border px-3 py-2 md:col-span-2" required />
