...
function yearFromMetadata(metadata) {
    const blobs = [metadata?.exif, metadata?.xmp, metadata?.iptc].filter(Boolean);
...
async function metadataYear(filePath) {
    try {
        return yearFromMetadata(await sharp(filePath).metadata());
    } catch {
        return null;
    }
}
        ...
        .upsert(
            {
                ...
                alt: item.alt?.el || item.alt?.de || '',
                alt_el: item.alt?.el || item.alt?.de || '',
                alt_de: item.alt?.de || item.alt?.el || '',
                year: item.year ?? null,
...
async function ensureSupabaseTag(name) {
    ...
    if (DRY_RUN) {
        return {
            id: 0,
            name_el: localized.el,
            name_de: localized.de
        };
    }
    ...
    const {data, error} = await supabase
        ...
        .upsert(
            {
                name: localized.el === localized.de ? localized.el : `${localized.el} / ${localized.de}`,
                name_el: localized.el,
                name_de: localized.de
            },
...
async function migrateImageItem({id, src, alt}) {
    ...
    return {
        ...
        videoSrc: '',
        alt: {el: alt, de: alt},
        year: await metadataYear(localFile),
