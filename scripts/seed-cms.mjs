...
async function createVariants(sharp, sourceUrl) {
    ...
    for (const size of SIZES) {
        try {
            ...
            await sharp(buffer, {failOn: 'none'}).resize({
                width: size,
                withoutEnlargement: true
            }).jpeg({quality: 84}).toFile(jpgPath);
            await sharp(buffer, {failOn: 'none'}).resize({
                width: size,
                withoutEnlargement: true
            }).webp({quality: 82}).toFile(webpPath);
...
async function run() {
    ...
    let current = {};
    try {
        const content = await fs.readFile(path.join(dataDir, 'cms.json'), 'utf-8');
        current = JSON.parse(content);
    } catch (e) {
        console.log('No existing cms.json found, creating a new one.');
    }
    const seeded = {...current, events, gallery};
    await fs.writeFile(path.join(dataDir, 'cms.json'), JSON.stringify(seeded, null, 2), 'utf-8');
